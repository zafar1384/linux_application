...
mqtt_broker_handle_t * mqtt_connect(const char* client, const char * hostname, uint32_t port)
{
	char server_ip[100];
...
//      fixed_header_t  fixed_header = { .QoS = 0, .connect_msg_t = CONNECT, .remaining_length = sizeof(var_header)+strlen(broker->clientid) };
        ...
        if( (uint64_t)send(broker->socket, packet, sizeof(packet), 0) < sizeof(packet)) 
        {
            close(broker->socket);
            free(broker);
            return 0;
...
void mqtt_display_message(mqtt_broker_handle_t *broker, int (*print)(int))
{
    ...
    SetSocketTimeout(broker->socket, 50000);

    while(1) 
    {
        // wait for next message
        int sz = recv(broker->socket, buffer, sizeof(buffer), 0);
        ...
        if(sz < 0)
        {
           printf("\nSocket recv returned %d, errno %d %s\n",sz, errno, strerror(errno));
        ...
        if(sz > 0) 
        {
            ...
            {                
                ...
                if (((buffer[0]>>1) & 0x03) > QoS0) 
                {
                    ...
                    if( (uint64_t)send(broker->socket, puback, sizeof(puback), 0) < sizeof(puback)) 
                    {
...
int mqtt_publish(mqtt_broker_handle_t *broker, const char *topic, const char *msg, QoS qos)
{
    ...
    else 
    {
        ...
        if(qos == QoS1)
        {
            ...
            {
                puts("failed to Publishing at QoS1");
                return -1;
            }
        }
...
void mqtt_disconnect(mqtt_broker_handle_t *broker)
{
    ...
    if ( (uint64_t)send(broker->socket, fixed_header, sizeof(fixed_header), 0)< sizeof(fixed_header)) 
    {
